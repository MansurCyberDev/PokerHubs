...
async def rules_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "📘 <b>How to play</b>\n\n"
        "1) Create a table in group with <code>/game</code>.\n"
        "2) Join via button, then open bot DM for actions.\n"
        "3) On your turn use check/call/fold/bet/raise/all-in.\n"
        "4) If timer ends, auto-fold is applied.\n"
        "5) Use <code>/leave</code> to leave current table.\n\n"
        "🍀 <b>Luck Boost</b>\n"
        "Admins can grant luck boost (0-100%) that improves your starting hands.\n"
        "Higher luck = better chance to get stronger hole cards.\n"
        "This affects only your initial 2 cards, not community cards."
        if lang == "en"
        else
        "📘 <b>Как играть</b>\n\n"
        "1) Создай стол в группе: <code>/game</code>.\n"
        "2) Садись за стол кнопкой и открой ЛС с ботом.\n"
        "3) На ходе доступны: чек/колл/фолд/ставка/рейз/олл-ин.\n"
        "4) Если таймер истекает — авто-фолд.\n"
        "5) Чтобы выйти со стола, используй <code>/leave</code>.\n\n"
    )
    await update.message.reply_text(text, parse_mode=ParseMode.HTML)
